...
func CreateBackup(w http.ResponseWriter, r *http.Request) {
	...
	c := cron.New()
	var spec string
	if second, ok := s.Backup.Schedule["second"].(float64); ok {
		spec = fmt.Sprintf("%.0f", second)
	} else {
		spec = " " + "*"
	}
	if min, ok := s.Backup.Schedule["minute"].(float64); ok {
		spec += " " + fmt.Sprintf("%.0f", min)
	} else {
		spec += " " + "*"
	}
	...
	pgHost := s.DockerNetwork + "_postgres_1"
	c.AddFunc(spec, backup.TriggerBackup(s.DockerNetwork, s.Backup.Dest.ApiKeySecret, s.Backup.Dest.ApiKeyID, pgHost, s.Db.Username, s.Db.Password, "postgres", s.Backup.Dest.BucketName))
	e := c.Entries()
	for _, v := range e {
		log.Println(v.Prev)
	}
